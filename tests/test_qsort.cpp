...
/*   Updated: 2025/12/09 18:15:56 by dlesieur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int run_int_case(const char *name, int *arr, size_t n)
{
	...
	if (n > 0)
	{
		work = (int *)dup_mem(arr, n, sizeof(int));
		expected = (int *)dup_mem(arr, n, sizeof(int));
		if (!work || !expected)
		{
			free(work);
			free(expected);
...
static int run_pair_case(const char *name, t_pair *arr, size_t n)
{
	...
	work = (t_pair*)dup_mem(arr, n, sizeof(t_pair));
	expected = (t_pair*)dup_mem(arr, n, sizeof(t_pair));
	if (!work || !expected)
	{
		free(work);
		free(expected);
...
int main(void)
{
	...
	/* random data */
	srand(0);
	rand_arr = (int *)malloc(nrand * sizeof(int));
	if (rand_arr)
	{
		for (i = 0; i < nrand; i++)
	...
	/* small arrays printed for visual verification */
	{
		...
		int *wrk = (int *)dup_mem(small, 6, sizeof(int));
		printf("\nvisual check before: ");
