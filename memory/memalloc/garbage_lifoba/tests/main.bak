...
static void test_generic_allocation(void)
{
	...
	printf("\n=== TEST 1: Generic Allocation (stalloc equivalent) ===\n");
	n = 10;
	int_array = (int *)st_alloc(n * sizeof(int));
	...
	while (i < n)
	{
		printf("%d%s", int_array[i], (i + 1 == n) ? "\n" : " ");
		i++;
	}
	point = (t_point *)st_alloc(sizeof(t_point));
	point->x = 42;
	point->y = 100;
	strcpy(point->label, "origin");
	printf("Point allocated: (%d, %d) '%s'\n", point->x, point->y,
		   point->label);
}
...
{
	t_stack_mark base_mark;
	int cmd;
	int i;
	t_stack_mark cmd_mark;
	t_node *nodes[5];

	printf("\n=== TEST 10: Parse Tree Simulation (dash usage pattern) ===\n");
	set_stack_mark(&base_mark);
	cmd = 0;
	while (cmd < 3)
	{
		...
		while (i < 5)
		{
			nodes[i] = (t_node *)st_alloc(sizeof(t_node));
			nodes[i]->type = 'N';
			nodes[i]->value = (char *)st_alloc(32);
			sprintf(nodes[i]->value, "node_%d_%d", cmd, i);
			if (i > 0)
				nodes[i - 1]->next = nodes[i];
			i++;
		}
		nodes[4]->next = NULL;
		printf("Command %d: built %d nodes\n", cmd, 5);
		printf("  First node value: %s\n", nodes[0]->value);
		pop_stack_mark(&cmd_mark);
		printf("Command %d cleaned up\n", cmd);
		cmd++;
	}
	pop_stack_mark(&base_mark);
	printf("All commands cleaned and stack reset\n");
}
