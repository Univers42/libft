...
/*   Updated: 2026/01/10 20:39:28 by marvin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
#define DEFAULT_WHITE 0xFFFFFF
     ...
     * @return Total length of src (strlen(src)). If return >= dsize, truncation occurred.
     */
    ...
    const char *const *find_string(const char *s, const char *const *array, size_t nmemb);
...
/* skip_trailing flags */
#define SKIP_TRIM_CHARS 0x01  /* treat pattern as set of chars to trim */
#define SKIP_TRIM_SUBSTR 0x02 /* treat pattern as whole substring to remove repeatedly from end */
#define SKIP_TRIM_WORDS 0x04  /* treat pattern as separators and remove trailing separator-only regions */
    ...
    char *skip_trailing_flags(char *line, const char *pattern_to_skip, int flags);
