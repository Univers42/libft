...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/12/14 01:16:05 by dlesieur          #+#    #+#             */
/*   Updated: 2025/12/14 02:35:49 by dlesieur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
#include "new_lib.h"
#include "ft_memory.h"
#include "ft_stdio.h" /* NOTE: removed from here previously to avoid transitive conflicts;
                      include ft_stdio.h directly only in sources that need it. */
#include "ft_string.h"
#include "rl_templates.h"
#include "error.h"
#include <errno.h>
#include "variables.h"
/* ensure var APIs and trap helpers are visible to readline modules */
#include "var.h"
#include "trap.h"

	/* Forward declarations for a few helpers used across readline files
	   (these may be defined in other modules; declaring them here avoids
	   implicit function warnings while building readline). */
	extern char *get_var(const char *name, int scope);
	extern int ft_set_var(const char *name, const char *value, int scope);
	extern char *ft_strsub(const char *s, unsigned int start, size_t len);
	extern char **get_vars_names_dar(const char *prefix, int mask);
	extern char **ft_strsplit(const char *s, char delim);
...
/* generic "no error" code used across readline (mapped to 0) */
#define NOERROR 0

/* Fallback mappings for setupterm() error codes expected by readline code.
   Map to standard errno macros so they can be passed to err_exit/rl_err.
   - EHRDCPY: hardware/capability copy error -> use EIO
   - ENTFND: terminfo entry not found -> use ENOENT
   - ENOTERMINFO: terminfo not available -> use ENOTTY
*/
#ifndef EHRDCPY
#define EHRDCPY EIO
#endif
#ifndef ENTFND
#define ENTFND ENOENT
#endif
#ifndef ENOTERMINFO
#define ENOTERMINFO ENOTTY
#endif
	...
	*/
	...
	/* Replacement wrapper to avoid conflicting `reset_shell_mode` symbol */
	void rl_reset_shell_mode(void);
	/* Add common utility prototypes used across readline sources */
	...
	*/
	...
	extern t_readline g_rl;
	extern unsigned char g_rl_flags;
