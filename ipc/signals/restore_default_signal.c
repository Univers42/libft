...
{
	if ((sig != DEBUG_TRAP && sig != ERROR_TRAP && sig != RETURN_TRAP) || (get_g_sig()->sigmodes[sig] & SIG_INPROGRESS) == 0)
...
{
	if ((get_g_sig()->sigmodes[sig] & SIG_HARD_IGNORE) || ((get_g_sig()->sigmodes[sig] & (SIG_TRAPPED | SIG_ASYNCSIG | SIG_NO_TRAP)) == SIG_ASYNCSIG))
	{
		/* store typed default handler */
