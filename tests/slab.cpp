...
char *ft_strjoin_slab(char *s1, const char *s2)
{
    ...
    new_str = (char *)slab_alloc(&slab, len1 + len2 + 1);
    if (!new_str)
        return (NULL);
    ptr = new_str;
    if (s1)
...
t_fd_list *ft_get_fd_node_slab(t_fd_list **fd_list, int fd)
{
    ...
    new_node = (t_fd_list *)slab_alloc(&slab, sizeof(t_fd_list));
    if (!new_node)
        return (NULL);
    new_node->fd = fd;
...
static char *ft_store_chunks(int fd, char *memory)
{
    ...
    buffer = (char *)slab_alloc(&slab, BUFFER_SIZE + 1);
    if (!buffer)
        return (NULL);
    while (ft_strclen(memory, '\n') == ft_strclen(memory, '\0'))
    {
        ...
        buffer[bytes_read] = '\0';
        memory = ft_strjoin_slab(memory, buffer);
        if (!memory)
            return (NULL);
    }
    return (memory);
...
static char *ft_get_line(char *memory)
{
    ...
    line = (char *)slab_alloc(&slab, newline_idx + 2);
    if (!line)
        return (NULL);
